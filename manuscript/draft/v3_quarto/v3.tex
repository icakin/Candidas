...
One qualification applies to several derived per-cell quantities.
Because these depend on the estimated starting cell number, the primary
among-taxon contrast is robust whereas the finer within-clade ranking is
conditional on the inoculum back-projection. We flag this recurring
distinction where it bears on a result and give its full treatment in
...
temperature-sensitive than respiration. (The absolute respiration
activation energies carry a temperature-structured uncertainty from the
inoculum back-projection, but the growth-minus-respiration ordering, the
comparison that matters here, is unaffected in every taxon.)
...
excluding a ratio of 1; Fig. 2d). All four C. \emph{auris} clades
credibly paid less at fever than C. \emph{parapsilosis} (cost ratios
0.43--0.72), the robust contrast. The finer ordering within C.
\emph{auris}, under which Clade IV is the cheapest, is conditional on
the inoculum back-projection (Robustness; Supplementary Fig. 6) and is
reported as such. Only two contrasts, Clade I versus Clade II (0.85,
0.68--1.04) and Clade I versus Clade III (1.18, 0.98--1.42), could not
be separated at three isolates per clade.
...
0.39, Clades I and III intermediate at 0.60 and 0.62, Clade IV highest
at 0.71), and of the three calibration parameters it was the only one
that separated the clades (5 of 6 pairwise contrasts with
non-overlapping 95\% CrI, versus 1 of 6 for the optimum shift and 0 of 6
for curvature; Supplementary Fig. 1b). Capacity behaved as a
reproducible clade-level trait, with 90\% of its variance falling
between clades (ICC₁ = 0.90; ANOVA F₃,₈ = 27.7, P = 1.4×10⁻⁴;
...
fourth of five), whereas the C. \emph{auris} versus C.
\emph{parapsilosis} contrast survives both bounds; we therefore state
the within-\emph{auris} ranking as conditional on the back-projection.
...
is, for every species measured, a carbon-inefficient environment. That
penalty is taxon-specific and tracks thermal adaptation: it is steepest
in the cold-adapted \emph{C. parapsilosis} and shallowest in the
warm-adapted \emph{C. auris} clades, so the \emph{C. auris} versus
\emph{C. parapsilosis} contrast is the firm biological result, while the
finer ranking among the \emph{C. auris} clades is conditional on the
inoculum back-projection.
