...
G = ROOT / 'gem'
if not (G / 'measured_tpc_honest.csv').exists():
...
hon = pd.read_csv(G / 'measured_tpc_honest.csv')
...
D = pd.DataFrame(rows)
out = G / ('loocv_nested_results_FAST.csv' if FAST else 'loocv_nested_results.csv')
