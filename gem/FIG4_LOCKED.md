...
|---|---|---|
| required *T*~m~ separation, fitted formulation | 32.54 °C | `gem/etcgem_counterfactual.py` |
| required *T*~m~ separation, corrected unfolding width (w = 2.2) | 18.0 °C | `gem/thermal_sensitivity.py` |
| required *T*~opt~ separation | 15.62 °C | `gem/etcgem_counterfactual.py` |
| paired Δ*T*~m~, *C. auris* − *C. haemulonii* | 0.411 °C [0.19, 0.63], n = 432 pairs, p = 2.2 × 10⁻⁴ | `gem/paired_dedup_audit.py` |
...
| model-predicted thermal limit | 52.7–54.5 °C across the four species | `gem/thermal_sensitivity.py` |
| held-out prediction at each relative's observed failure temperature | 0.79–0.85 of the model's own peak | `gem/loocv_nested.py` |
...
protein by up to 5.36 °C, sd 1.27 °C, on 200 *C. auris* enzymes. `gem/thermal_tm.csv` was
...
confirmed: `gem/run_seq2tm.py --seqs-from ... --batch-size 1` followed by
`gem/padding_sensitivity.py` tests whether the artefact is biased per species. Until that
