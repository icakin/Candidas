...
G = ROOT / "gem" if (ROOT / "gem").is_dir() else ROOT
THR = 0.05
...
calib = json.load(open(G / 'etcgem_calib.json'))
SIG, W0, P0, SCALE = calib['sig'], calib['w'], calib['P'], calib['SCALE']
topt = pd.read_csv(G / 'thermal_topt.csv'); tm = pd.read_csv(G / 'thermal_tm.csv')
for df in (topt, tm):
...
def build(sp, aa_bound=3.0, block_egc=False):
    xml, medf = SP[sp]
    m = cobra.io.read_sbml_model(str(G / 'models' / xml)); med = pd.read_csv(G / medf)
    ...
    mw = {r['gene']: float(r['MW_kDa']) for _, r in pd.read_csv(G / f'enzyme_mw_{sp}.csv').iterrows()}
    kc = pd.read_csv(G / f'kcat_reaction_{sp}.csv')
...
print(gl)
pd.DataFrame(rows).to_csv(G / 'gap_robustness_results.csv', index=False)
