...
#   4. verifies every package the pipeline actually loads can be loaded
...
#
# It does NOT touch the Python side. That is:
...
# Kept in sync by hand with `grep -ohE 'library\(|requireNamespace\(|::' scripts/*.R`.
NEEDED <- c(
  ...
  # fitting / numerics
  "minpack.lm", "zoo",
  # the four click-driven apps
  "shiny", "rstudioapi",
  # Bayesian stage
  "brms", "posterior",
  # 11_capacity_expression.R (Bioconductor)
  "edgeR",
  # xlsx ingest in 01
  "readxl"
)
...
# WHICH BACKEND? 08_bayesian_models.R calls brms::brm() without a `backend`
...
message("  ==> 08_bayesian_models.R will use backend: ", backend)
...
"
  ...
  if (inherits(res, "error")) {
    ...
    message("\n  ", strrep("-", 70))
    message("  THE BAYESIAN STAGE (08_bayesian_models.R) CANNOT RUN. Fix this first.")
    message("  ", strrep("-", 70))
    ...
    message("      options(brms.backend = 'cmdstanr')   # in ~/.Rprofile or before sourcing 08")
...
message("  Next:")
message("    1. Python side :  cd cauris_etcgem && python3 -m venv .venv \\")
message("                        && .venv/bin/python -m pip install -r requirements.lock")
