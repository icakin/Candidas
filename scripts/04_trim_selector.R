...
# - Curves are keyed by T + OTU + Replicate (OTU = Candida isolate code 1..18).
...
# is kept IDENTICAL to 07_oxygen_fits.R's fit_one, so the blue guide line is the
...
# Candida isolate code, integer 1..18). Curves are keyed by T + OTU + Replicate,
...
long <- readr::read_csv(long_path, show_col_types = FALSE) %>%
  dplyr::mutate(T = as.numeric(T), OTU = as.integer(OTU),
                ...
                key = paste(T, OTU, Replicate, sep = "_"))
...
# Isolate display names typed in 01_convert_xlsx.R (tables/otu_names.csv). The
# dropdown and plot titles show these names instead of the numeric OTU code;
# the saved CSV blocks still use the OTU code underneath (that is the key).
otu_name_map <- stats::setNames(character(0), character(0))
...
if (file.exists(.names_csv)) {
  ...
  if (!is.null(.nm) && all(c("OTU", "otu_name") %in% names(.nm))) {
    otu_name_map <- stats::setNames(trimws(as.character(.nm$otu_name)),
                                    as.character(as.integer(.nm$OTU)))
  }
}
otu_disp <- function(o) {
  nm <- unname(otu_name_map[as.character(o)])
  ifelse(is.na(nm) | !nzchar(nm), paste0("OTU", o), nm)
}

# Order OTUs ascending (numeric).
.otu_key <- function(x) suppressWarnings(as.numeric(x))
curves <- long %>%
  dplyr::distinct(T, OTU, Replicate, key) %>%
  dplyr::arrange(T, .otu_key(OTU), Replicate) %>%
  dplyr::mutate(label = sprintf("%s  |  T=%g  |  %s", otu_disp(OTU), T, Replicate))
...
if (file.exists(out_csv)) {
  ...
  if (!is.null(prev) && all(c("T", "OTU", "Replicate") %in% names(prev))) {
    init <- prev %>%
      dplyr::mutate(key = paste(as.numeric(T), as.integer(OTU), toupper(Replicate), sep = "_"),
...
# tables/plot_exclude_points.csv (seeded from PLOT_EXCLUDE_POINTS in 07_oxygen_fits.R).
...
if (file.exists(excl_csv)) {
  ...
  if (!is.null(pe) && all(c("T", "OTU", "Replicate") %in% names(pe))) {
    excl_init <- paste(as.numeric(pe$T), as.integer(pe$OTU), toupper(pe$Replicate), sep = "_")
  }
}
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      width = 4,
      # With ~1080 curves the full list is unusable. Filter it down first.
      fluidRow(
        column(6, selectInput("flt_iso", "Isolate:",
                              choices = c("All", sort(unique(as.character(
                                otu_disp(curves$OTU))))),
                              selected = "All")),
        column(6, selectInput("flt_T", "Temperature:",
                              choices = c("All", sort(unique(curves$T))),
                              selected = "All"))
      ),
...
server <- function(input, output, session) {
  ...
  observeEvent(list(input$flt_iso, input$flt_T, input$flt_status, input$refresh_filter), {
    cc <- curves
    if (!is.null(input$flt_iso) && input$flt_iso != "All") {
      cc <- cc[otu_disp(cc$OTU) == input$flt_iso, , drop = FALSE]
    }
    if (!is.null(input$flt_T) && input$flt_T != "All") {
      cc <- cc[cc$T == as.numeric(input$flt_T), , drop = FALSE]
    }
    fs <- input$flt_status
  ...
  observeEvent(input$stab_iso, {
    target <- if (!is.null(input$flt_iso) && input$flt_iso != "All") {
      ...
      if (nrow(cur) == 0) NA_character_ else otu_disp(cur$OTU[1])
    }
    ...
    grp <- curves[otu_disp(curves$OTU) == target, , drop = FALSE]
    if (nrow(grp) == 0) {
  ...
  win_df <- reactive({
    w <- wins()
    if (nrow(w) == 0) return(curves[0, c("T", "OTU", "Replicate")] %>%
                               dplyr::mutate(fit_start = numeric(0), fit_end = numeric(0)))
    curves %>% dplyr::inner_join(w, by = "key") %>%
      dplyr::arrange(T, .otu_key(OTU), Replicate) %>%
      dplyr::select(T, OTU, Replicate, fit_start, fit_end)
  })
  ...
  excl_df <- reactive({
    keys <- excl()
    if (length(keys) == 0) return(curves[0, c("T", "OTU", "Replicate")])
  ...
  output$combo <- renderText({
    ...
    if (nrow(e) == 0) {
      excl_txt <- paste0("PLOT_EXCLUDE_POINTS <- data.frame(\n",
        "  T = numeric(0), OTU = integer(0), Replicate = character(0),\n",
        "  stringsAsFactors = FALSE\n)")
    } else {
      excl_txt <- paste0(
        "PLOT_EXCLUDE_POINTS <- data.frame(\n",
        "  T         = c(", paste(e$T, collapse = ", "), "),\n",
        "  OTU       = c(", paste(e$OTU, collapse = ", "), "),\n",
...
# Rscript. results/tables/manual_fit_windows.csv and plot_exclude_points.csv
# are committed INPUTS (config.R reads them when USE_APP_TRIM_FILES is TRUE).
...
#   Terminal, on purpose  -> Rscript scripts/04_trim_selector.R --app
#                            (or CANDIDAS_RUN_APP=1 Rscript scripts/04_trim_selector.R)
...
if (.candidas_run_app()) {
  ...
  message("04_trim_selector.R: headless - ui/server defined, app NOT launched, ",
          ...
          "plot_exclude_points.csv are read as data by config.R).\n",
