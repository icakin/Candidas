...
# --file= is checked FIRST (C1): under `Rscript scripts/06_inoculation.R` neither
# sys.frame(1)$ofile nor rstudioapi is available, so this used to fall through to
# getwd() and then fail on "cannot open file .../config.R". source()ing from
# run_all.R was unaffected, which is why the bug stayed hidden.
.this_dir <- local({
...
# ---- headless guard (C1) ----------------------------------------------------
# THIS is the one that made run_all.R hang: run_all.R source()s 06 in its loop,
# and under Rscript the old `else` branch called runApp(), so the master runner
# blocked on a Shiny server forever and never reached 07 onwards. Sourcing this
# file now defines ui/server and returns. tables/otu_inoc.csv is a committed
# INPUT and is not regenerated by an unattended run.
...
if (.candidas_run_app()) {
  ...
  message("06_inoculation.R: headless - ui/server defined, app NOT launched, ",
          "nothing written (tables/otu_inoc.csv is read as data by 07).\n",
